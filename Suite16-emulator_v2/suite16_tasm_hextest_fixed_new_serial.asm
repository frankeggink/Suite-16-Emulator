...
; assemble with command: tasm -s -h -c -g0 -t16 suite16_tasm_hextest_fixed_new_serial.asm out.hex
; generate MEM-array with command: binToMem.exe < suite16_tasm_hextest_fixed_new_serial.lst > output
...
start:
	...
	SET R1, 512				; location where serial character is always put
	SET R2, 512+1			; start of buffer
	
	
not_done:	
	IN						; Getchar
	LD R0, @R1				; received char is now in R[0]
	ST R7, R0				; save R[0] in R[7], need to use it lateron
	SBI $D					; is char a \r
	BEQ done				; we are done
	LD R0, R7				; get back value
	SBI $A					; is char a \n
	BEQ done				; we are done
	LD R0, R7				; get back value
	ST @R2, R0				; store received serial character in buffer
	...
	SET R1, 512+1		    ; R1 points to first entry in char buffer
	SET R0, $0
	;INC R2
	ST @R2, R0
...
PRINTNUM:
	...
	BLT signedToUnsigned ; test for negative number
	BRA unsigned		 ; already signed number
	
signedToUnsigned:		 ; substract 30000
	SET R1, 30000
	SUB R1
		
	INC R2				 ; store a 3 as first digit
	INC R2
	INC R2
unsigned:	
	SET R1, 10000
...
GETHEX:
	...
	SET R1, 512+1				; R1 is start of text buffer
	SET R7, 0				; Clear R7
